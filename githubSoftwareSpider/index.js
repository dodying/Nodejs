...
const main = async () => {
  ...
  while ([].concat(...Object.values(database.wait)).length) {
    if (database.wait.repo.length) {
      let repo = database.wait.repo[0]
      if (!database.done.repo.includes(repo)) {
        let res = await req(`https://api.github.com/repos/${repo}/releases`)
      ...
      let user = database.wait.user[0]
      if (!database.done.user.includes(user)) {
        let res = await req(`https://api.github.com/users/${user}/repos`)
        let repos = res.json.map(i => i.full_name)
      ...
      let user = database.wait.star[0]
      if (!database.done.star.includes(user)) {
        let res = await req(`https://api.github.com/users/${user}/starred`)
        let repos = res.json.map(i => i.full_name)
        database.wait.repo.push(...repos)
        let users = res.json.map(i => i.owner.login)
