 ...
 */
  ...
  function matcherFromGroupMatchers(elementMatchers, setMatchers) {
    ...
    const superMatcher = function (seed, context, xml, results, outermost) {
      ...
      const dirrunsUnique = (dirruns = dirruns + (contextBackup == null ? 1 : Math.random() || 0.1));
 ...
 */
  ...
  // Document location
  const ajaxLocation = window.location.href;
  ...
  jQuery.extend({
    ...
    ajax(url, options) {
      ...
      if (!s.hasContent) {
        ...
        if (s.data) {
          cacheURL = (s.url = s.url + ((rquery.test(cacheURL) ? '&' : '?') + s.data));
  ...
  jQuery.fn.extend({
    ...
    position() {
      ...
      if (jQuery.css(elem, 'position') === 'fixed') {
        ...
        parentOffset.top = parentOffset.top + jQuery.css(offsetParent[0], 'borderTopWidth', true);
        parentOffset.left = parentOffset.left + jQuery.css(offsetParent[0], 'borderLeftWidth', true);
      }
