...
const _ = {
  ...
  specialRule: [{
    mode: 0, //Strict Mode: 1 means one condition matches, 0 each matches (tag includes), 2 each (tag equal)
    artist: 'example',
...
const sortFileBySpecialRule = info => {
  ...
  for (let i = 0; i < rule.length; i++) {
    ...
    if (filter) return path.resolve(_.libraryFolder, _.specialFolder, escape(folder));
  }
  return false;
}
const sortFile = info => {
...
if (fs.exists('config.js')) {
  ...
  Object.assign(_, config.default);
}

const main = async () => {
  //读取列表
  let lst = glob.sync(_.comicFolder + (_.globRecursive ? '\\**' : '') + '\\*.@(zip|cbz)');
