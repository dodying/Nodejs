...
let database;
const databaseFile = path.resolve(_.libraryFolder, 'database.json');
const main = async () => {
  ...
  fs.watch(_.libraryFolder, { recursive: true }, (event, filename) => {
    ...
    const filepath = path.join(_.libraryFolder, filename);
    const name = nameParsed(filename);
