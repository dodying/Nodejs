...
const main = async () => {
  const exts = ['.zip', '.cbz'];
  const toExt = exts.includes(process.argv[2]) ? process.argv[2] : '.zip';
  ...
  for (const file of files) {
    if (fs.statSync(file).isFile()) {
      ...
      const nameNew = path.join(dir, name + toExt);
      if (nameNew !== file) {
      ...
      const list = fs.readdirSync(file).map(i => fs.statSync(path.join(file, i)).mtime).sort();
      if (!list.length) continue;
      const time = list[list.length - 1];
