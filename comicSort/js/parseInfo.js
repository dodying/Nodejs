...
const parseInfo = text => {
  ...
  if (text.match(/Downloaded at (.*)/)) {
    ...
    downloadTime = new Date(downloadTime).toLocaleString('zh-cn', {
      hour12: false
    })
  } else {
  ...
  let output = {
    title: lines[0],
    jTitle: (lines[1].match(/^http/)) ? lines[0] : lines[1],
    page: [],
    downloadTime,
    tagString: []
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].match(/^Page \d+:/)) {
      ...
      let re = lines[i].match(/^Image (\d+):\s+(.*)$/)
  ...
  for (let i = 0; i < lines.length; i++) {
    let lineThis = lines[i]
    let arr = lineThis.split(': ')
    if (arr.length > 1) {
  ...
  if ('parody' in info && info['parody'].length === 1 && info['parody'].includes('original')) delete info['parody']
