...
const main = async () => {
  ...
  program.command('run [urls...]', { isDefault: true }).action(async (urls) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      for (const lib of $libs) {
        const $info = { name: lib.name, tryOtherLib: lib.tryOtherLib, ...await getRemoteInfo(url, lib) };
        ...
        if (check === true) {
          ...
          if (info.chapters) {
            fs.writeFileSync('./ffmetadata', [
              ...
              '',
              ...JSON.parse(info.chapters).map((i) => `[CHAPTER]\nTIMEBASE=1/1000\nSTART=${i.start}\nEND=${i.start}\ntitle=${i.title}\n`),
            ].join('\n'));
  ...
  program.command('test [names...]').description('test libs that filtered by names').action(async (names) => {
    ...
    for (const lib of $libs) {
      ...
      const info = { name: lib.name, ...await getRemoteInfo(url, lib) };
