...
// Main
const main = async () => {
  let data = fse.existsSync('data.json') ? fse.readJSONSync('data.json') : { items: [], files: [], manual: [] }
  data.manual = []
  let gdriveDir = []
  let gdrive = []
  let mega = []
  ...
  let gdriveDirAll = fse.existsSync('all-gdriveDir.txt') ? fse.readFileSync('all-gdriveDir.txt', 'utf-8').split(/\r\n/) : []
  ...
  let gdriveAll = fse.existsSync('all-gdrive.txt') ? fse.readFileSync('all-gdrive.txt', 'utf-8').split(/\r\n/) : []
  ...
  let megaAll = fse.existsSync('all-mega.txt') ? fse.readFileSync('all-mega.txt', 'utf-8').split(/\r\n/) : []
